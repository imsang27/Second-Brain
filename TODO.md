...
- [ ] **데이터 인제스션(Ingestion) 파이프라인**
    - [ ] Markdown 로더 구현 (날짜, 태그 등 메타데이터 추출 포함)
    - [ ] 텍스트 청킹(Chunking) 전략 수립 (사고의 맥락이 끊기지 않는 단위 설정)
- [ ] **Vector DB 구축**
    - [ ] bge-m3 임베딩 모델 연동함
    - [ ] ChromaDB를 활용한 로컬 벡터 저장소 초기화 및 데이터 로드함
- [ ] **기본 검색 및 응답**
    - [ ] 시멘틱 검색(Semantic Search) 로직 구현함
    - [ ] 검색된 기록을 바탕으로 LLM 답변 생성 프롬프트 작성함
